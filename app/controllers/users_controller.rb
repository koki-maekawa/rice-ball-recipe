class UsersController < ApplicationController
  ...
  def show
    @created_rice_balls = @user.rice_balls.includes(:image_attachment).order(created_at: :desc).limit(4)
    @created_rice_balls_count = @user.rice_balls.size
    @bookmarked_rice_balls = @user.bookmarked_rice_balls.includes(:image_attachment, :user).order("bookmarks.created_at DESC").limit(4)
